...
import { bericht, verschuldigd, TEST_ORIGIN, TEST_PROJECT, type Soort } from '../_shared/meldingen.ts';
...
type Planning = { match_key: string; member_id: string; tegenstander: string; aftrap: string | null; score_at: string | null; deadline: string; antwoord: boolean; aanwezig: boolean; gestemd: boolean; eerste_verzonden: string | null };
...
async function verzendRij(config: any) {
  ...
  for (const j of jobs) {
    ...
    for (const sub of subs) {
      try {
        const inhoud = bericht(j.soort,p.tegenstander);
