...
export function TwizzitBeheer(){
 ...
 return <section className="kaart"><h2>Twizzit bijwerken</h2><p className="klein zacht">{status?.laatste_succes?'Laatste update: '+new Date(status.laatste_succes).toLocaleString('nl-BE'):'Nog geen automatische update.'}</p><button className="knop licht" disabled={bezig||status?.wacht||status?.bezig} onClick={async()=>{setBezig(true);await laad(true);setBezig(false);}}>{status?.bezig?'Bezig met ophalen…':status?.wacht?'Update aangevraagd':'Nu bijwerken'}</button><p className="klein zacht">Aanvragen worden ongeveer elk kwartier opgehaald. GitHub kan vertraging hebben. Heropen daarna de kalender voor de nieuwste gegevens.</p>{(fout||status?.fout)&&<p role="alert">{fout||status?.fout}</p>}</section>;
}
