...
export function Tactiekbord({formatie='4-3-3',namen,compact=false,slotjes=[],zaal=false,icoontjes=()=>null,melding=null}: Omit<Props,'memberIds'> & {zaal?:boolean;icoontjes?:(pos:string)=>import('react').ReactNode;melding?:import('react').ReactNode}) {
  const label=(pos:string)=>zaal?({GK:'Doelvrouw',LB:'Linksachter',RB:'Rechtsachter',LW:'Linksvoor',RW:'Rechtsvoor'}[pos]??pos.replace('BANK','Bank ')):positieLabel(pos);
  const bank=zaal?['BANK1','BANK2','BANK3','BANK4','BANK5']:BANK;
  const plekken = (zaal ? [['GK'],['LB','RB'],[],['LW','RW']] : FORMATIES[formatie]).flatMap((rij, r) => rij.map((pos, i) => ({ pos, x:100*(i+1)/(rij.length+1), y:[85,65,43,20][r] })));
  return <section className={"tactiekbord" + (compact ? " compact" : "")} aria-label={`Opstelling ${zaal ? 'zaalvoetbal' : formatie}`} data-zaal={zaal || undefined}>
    {melding}
    <div className="tactiek-kop"><strong>{zaal?'STECA VROUWEN':'STECA JUNIORS'}</strong><span>{zaal?'2–2':formatie}</span></div>
    ...
    <div className="reserve-scene">
      ...
      {bank.map((pos,i)=><div key={pos} className="bankspeler" style={{left:(zaal?[15,32.5,50,67.5,85]:[23,40,57,87])[i]+"%"}}><Shirt label={String(i+1)} />{slotjes.includes(pos) && <i className="veld-slot" aria-label="Vergrendeld"><LockSimple size={14} weight="fill" /></i>}<div className="speler-naamblok">{icoontjes(pos)}<span>{namen[pos] ?? `Bank ${i+1}`}</span></div></div>)}
