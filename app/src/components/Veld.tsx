...
type Props = { formatie: Formatie; namen: Record<string, string | undefined>; compact?: boolean; slotjes?: string[] };
...
export function Veld({ formatie, namen, compact = false, slotjes = [] }: Props) {
  ...
  return <section className={"tactiekbord" + (compact ? " compact" : "")} aria-label={`Opstelling ${formatie}`}>
    ...
    <div className="reserve-scene">
      ...
      {BANK.map((pos,i)=><div key={pos} className="bankspeler" style={{left:[23,40,57,87][i]+"%"}}><Shirt label={String(i+1)} />{slotjes.includes(pos) && <i className="veld-slot" aria-label="Vergrendeld"><LockSimple size={14} weight="fill" /></i>}<span>{namen[pos] ?? `Bank ${i+1}`}</span></div>)}
