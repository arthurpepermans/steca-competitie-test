...
function Pronostieken({ openbaar,klassement }: {openbaar:boolean;klassement:boolean}) {
 ...
 {klassement?<div className="kaart tabel-scroll"><table><thead><tr><th>#</th><th>Naam</th><th>Punten</th><th>Exact</th><th>Verschil</th><th>Winnaar</th><th>Matchen</th></tr></thead><tbody>{rang.data?.map((r,_i,rs)=><tr key={r.user_id}><td>{rs.findIndex(x=>x.punten===r.punten)+1}</td><td>{r.naam}{r.user_id===session?.user.id?' (jij)':''}</td><td><strong>{r.punten}</strong></td><td>{r.exact}</td><td>{r.verschil}</td><td>{r.winnaar}</td><td>{r.gespeeld}</td></tr>)}</tbody></table>{!rang.laden&&!rang.data?.length&&<p>Nog geen pronostieken voor dit seizoen. Wie begint?</p>}</div>:
