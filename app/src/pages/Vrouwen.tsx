...
export function Vrouwen() {
  ...
  return <>
    ...
    <main className="inhoud v-inhoud">{clubActief&&<VrouwenClub tab={tab} detail={pathname.split('/')[3]} />}
    ...
    <nav ref={navRef} className="nav" aria-label="Vrouwen hoofdnavigatie">{[{to:'/vrouwen',label:'Home',Icon:House},{to:'/vrouwen/kalender',label:'Kalender',Icon:CalendarDots},{to:'/vrouwen/klassement',label:'Klassement',Icon:Trophy},...(clubActief?[{to:'/vrouwen/opstelling',label:'Opstelling',Icon:SoccerBall},{to:'/vrouwen/kantine',label:'Kantine',Icon:BeerStein},{to:'/vrouwen/leden',label:'Leden',Icon:UsersThree}]:[])].map(({to,label,Icon})=><NavLink key={to} to={to} end className={({isActive})=>isActive?'actief':''}><Icon size={25}/><span>{label}</span></NavLink>)}</nav>
  </>;
}
