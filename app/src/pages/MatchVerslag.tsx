...
import { bewaarVerslag, haalVerslagen, metVerslag, samenvatting, sorteerMomenten, type Moment, type Verslag } from '../lib/matchverslag';
...
export function MatchVerslag() {
  ...
  return <>
    ...
    <article className="matchverslag">
      ...
      {verslag?.momenten.length ? <ol className="match-tijdlijn">{sorteerMomenten(verslag.momenten).map((m, i) => <li key={i} className={`moment ${m.kant}`}>
        ...
        <span className="moment-minuut">{m.soort === 'goal' ? <SoccerBall size={22} weight="duotone" /> : <i className={`moment-kaart ${m.soort}`} />}{m.minuut === null ? '—' : `${m.minuut}′`}</span>
      </li>)}<li className="moment-aftrap">AFTRAP</li></ol> : <div className="verslag-totalen">{totalen.length ? <><p className="klein zacht">Wedstrijdminuten zijn niet ingevuld. Hieronder staan de geregistreerde totalen.</p>{totalen.map(s => <div className="verslag-totaal" key={s.member_id}><strong>{s.naam}</strong><span>{[s.goals && `${s.goals} goals`, s.assists && `${s.assists} assists`, s.geel && `${s.geel} geel`, s.rood && `${s.rood} rood`].filter(Boolean).join(' · ')}</span></div>)}</> : <p>Nog geen goals, assists of kaarten ingevuld.</p>}</div>}
...
function VerslagInvoer({ match, verslag, namen, klaar }: { match: Match; verslag?: Verslag; namen: string[]; klaar: () => Promise<void> }) {
  ...
  async function opslaan(e: FormEvent) { e.preventDefault(); setBezig(true); setFout(''); try { await bewaarVerslag(match.match_key, thuis, uit, momenten, verslag?.updated_at ?? null); await klaar(); } catch(e) { setFout(foutTekst(e)); } finally { setBezig(false); } }
  return <form onSubmit={opslaan} className="verslag-invoer"><Fout tekst={fout} /><fieldset disabled={bezig}><legend>Uitslag</legend><div className="verslag-scoreinvoer"><label>{match.thuis}<input aria-label="Thuisscore" type="number" min="0" max="99" required value={thuis} onChange={e => setThuis(Number(e.target.value))} /></label><label>{match.uit}<input aria-label="Uitscore" type="number" min="0" max="99" required value={uit} onChange={e => setUit(Number(e.target.value))} /></label></div>
    ...
    {momenten.map((m, i) => <fieldset className="verslag-momentinvoer" key={i}><legend>Moment {i + 1}</legend>
      <label>Minuut (optioneel)<input type="number" min="0" max="130" value={m.minuut ?? ''} onChange={e => wijzig(i, { minuut: e.target.value === '' ? null : Number(e.target.value) })} /></label>
