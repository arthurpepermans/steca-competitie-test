...
export function MatchVerslag() {
  ...
  return <>
    ...
    {rechten(lid).isAdmin && (/^(test-invoer-|push-test-)/.test(key) || key==='test-matchverslag-voorbeeld') && <section className="kaart"><Fout tekst={wisFout} /><button className="knop licht" disabled={wissen} onClick={async()=>{if(!confirm('Deze testmatch met verslag, stemmen en sfeerbeelden verwijderen?'))return;setWissen(true);setWisFout('');try{await verwijderTestmatch(key);navigate('/kalender');}catch(e){setWisFout(foutTekst(e));setWissen(false);}}}>{wissen ? 'Testmatch verwijderen…' : 'Testmatch verwijderen'}</button></section>}
